...
configure_zshrc() {
  ...
  local local_override="${HOME}/.zshrc.local"
  local backup="${target}.pre-install-backup"

  if [ -f "$target" ]; then
    if [ "$BACKUP_EXISTING" = "true" ] && [ ! -f "$backup" ]; then
      log "Backing up ~/.zshrc to ~/.zshrc.pre-install-backup"
      cp "$target" "$backup"
    fi
    if [ ! -f "$local_override" ]; then
      preserve_user_config "$target" "$local_override"
    else
      log "~/.zshrc.local exists; keeping your config unchanged"
    fi
  fi
  ...
  if [ -f "${REPO_DIR}/zshrc" ]; then
    ...
    cat >"$target" <<'EOF'
# ~/.zshrc generated by zshrc bootstrap
...
main() {
  ...
  local backup_path="${HOME}/.zshrc.pre-install-backup"
  local had_backup=false
  [ -f "$backup_path" ] && had_backup=true
...
STEP 2 - Set font (required for icons)
  Terminal / iTerm2 / VS Code / Cursor: Font = "MesloLGS NF"

STEP 3 (optional) - Reconfigure prompt
  p10k configure

--------------------------------------------------------------------------------
  Backup (if anything is broken, restore from here)
--------------------------------------------------------------------------------
EOF
  if [ "$had_backup" = true ]; then
    cat <<EOF
  Backup path:        $backup_path
  Restore command:   cp $backup_path ~/.zshrc
EOF
  else
    echo "  (No backup: this was a fresh install or backup already existed)"
  fi
  cat <<EOF

--------------------------------------------------------------------------------
  What the new ~/.zshrc includes
--------------------------------------------------------------------------------
  1. Our tools:       Powerlevel10k, Oh My Zsh, fzf, eza, bat, lazygit, thefuck, mygit
  2. Your config:     ~/.zshrc.local (paths, env vars, aliases - sourced at end)

  Your env vars and PATH entries from your original config are in ~/.zshrc.local.
  Both run: our tools first, then yours. Nothing is lost.

--------------------------------------------------------------------------------
  Installed
--------------------------------------------------------------------------------
  Homebrew, Python, Node.js, Oh My Zsh, Powerlevel10k, MesloLGS NF
  fzf, ripgrep, fd, eza, bat, thefuck, lazygit
...
--------------------------------------------------------------------------------
  Commands
--------------------------------------------------------------------------------
  mygit [project]     go to ~/dev/[project]
  mygit -n [project]  create new project and open in editor
  f / fuck            fix last command typo
  lg                  lazygit
  ls / ll / tree      eza (when installed)

--------------------------------------------------------------------------------
  Files
--------------------------------------------------------------------------------
  ~/.zshrc            our tools + sources ~/.zshrc.local at end
  ~/.zshrc.local      your paths, vars, aliases (never overwritten)
EOF
  if [ "$had_backup" = true ]; then
    echo "  $backup_path  your original before this run"
  fi
  cat <<EOF
