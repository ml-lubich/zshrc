...
- **`p10k.zsh`**: Powerlevel10k prompt configuration that is copied to `~/.p10k.zsh` (only if it doesn't exist, preserving user customizations).

### Responsibilities

- **Bootstrap layer**: `scripts/install.sh` (imperative operations, installs and configures tools).
- **Shell configuration layer**: `zshrc` and `p10k.zsh` (user-level shell behavior).
...
- **`p10k.zsh`**: Only copied to `~/.p10k.zsh` if it doesn't exist (preserves user customizations).
